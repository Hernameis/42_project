...
/*   Updated: 2021/03/18 15:00:00 by sunmin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	win->data = (int *)mlx_get_data_addr(win->img_ptr,\
			&win->bpp, &win->size_l, &win->endian);
	win->wall_n_ptr = mlx_xpm_file_to_image(win->mlx,\
			win->wall_n_addr, &win->wall_n_width, &win->wall_n_height);
	win->wall_n_data = (int *)mlx_get_data_addr(win->wall_n_ptr,\
		   	&win->wall_n_bpp, &win->wall_n_size_l, &win->wall_n_endian);
	win->wall_s_ptr = mlx_xpm_file_to_image(win->mlx, win->wall_s_addr,\
		   	&win->wall_s_width, &win->wall_s_height);
	win->wall_s_data = (int *)mlx_get_data_addr(win->wall_s_ptr,\
		   	&win->wall_s_bpp, &win->wall_s_size_l, &win->wall_s_endian);
	win->wall_e_ptr = mlx_xpm_file_to_image(win->mlx, win->wall_e_addr, \
			&win->wall_e_width, &win->wall_e_height);
	win->wall_e_data = (int *)mlx_get_data_addr(win->wall_e_ptr, \
			&win->wall_e_bpp, &win->wall_e_size_l, &win->wall_e_endian);
	win->wall_w_ptr = mlx_xpm_file_to_image(win->mlx, win->wall_w_addr, \
			&win->wall_w_width, &win->wall_w_height);
	win->wall_w_data = (int *)mlx_get_data_addr(win->wall_w_ptr, \
			&win->wall_w_bpp, &win->wall_w_size_l, &win->wall_w_endian);
	win->sprite_ptr = mlx_xpm_file_to_image(win->mlx, win->sprite_addr, \
			&win->sprite_width, &win->sprite_height);
