...
/*   Updated: 2021/04/05 20:06:05 by sunmin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		key_pressed_ws(t_win *win)
{
	if (win->press_w)
	{
		if (if_pass_wall(win, KEY_W) == 1 && if_pass_sprite(win, KEY_W) == 1)
	...
	if (win->press_s)
	{
		if (if_pass_wall(win, KEY_S) == 1 && if_pass_sprite(win, KEY_S) == 1)
			win->player_x -= cos(win->player_dir) * win->player_move_speed;
		if (if_pass_wall(win, KEY_S) == 1 && if_pass_sprite(win, KEY_S) == 1)
			win->player_y -= sin(win->player_dir) * win->player_move_speed;
	}
...
void		key_pressed_qe(t_win *win)
{
	if (win->press_q)
	{
		...
		if (if_pass_wall(win, KEY_Q) == 1 && if_pass_sprite(win, KEY_Q) == 1)
			win->player_x += cos(win->player_dir) * win->player_move_speed;
		if (if_pass_wall(win, KEY_Q) == 1 && if_pass_sprite(win, KEY_Q) == 1)
			win->player_y += sin(win->player_dir) * win->player_move_speed;
	...
	else if (win->press_e)
	{
		win->temp = win->player_dir;
		win->player_dir -= M_PI / 2;
		if (if_pass_wall(win, KEY_E) == 1 && if_pass_sprite(win, KEY_E) == 1)
			win->player_x -= cos(win->player_dir) * win->player_move_speed;
		if (if_pass_wall(win, KEY_E) == 1 && if_pass_sprite(win, KEY_E) == 1)
			win->player_y -= sin(win->player_dir) * win->player_move_speed;
		win->player_dir = win->temp;
